...
class UsersController extends Controller
{
    ...
    {
        // منع أي مستخدم مسجل دخول من الوصول لصفحة التسجيل
        if (auth()->check()) {
            return redirect('/');
        }
    ...
    public function save(Request $request)
    {
        ...
        // التحقق من كلمة المرور فقط إذا تم إدخال قيمة جديدة
        ...
        // فقط الـ Admin يقدر يعدّل الأدوار والـ permissions
    ...
    {
        if (!auth()->user()->hasPermissionTo('create_employee')) abort(401);
    ...
    public function listCustomers(Request $request)
{
    ...
    //     // Debugging: Log the roles of the user
    //     $userRoles = $user->roles->pluck('name')->toArray();
    //     \Log::info('User roles for ID ' . $user->id . ': ' . json_encode($userRoles));
    ...
    public function addCredit(Request $request, User $user)
{
    ...
    // التحقق من الدور باستخدام hasRole
    if (!$user->hasRole('Customer')) {
        \Log::error('Attempt to add credit to non-customer: User ID ' . $user->id . ' does not have Customer role.');
        return redirect()->back()->withErrors('You can only add credit to customers.');
    }
